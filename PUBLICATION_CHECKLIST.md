...
配布元の作業リポジトリには、個人利用のキャラクター画像が含まれる場合がある。
そのため、配布元をそのまま public に切り替えず、履歴を持たない新しいスナップショットを使う。
この公開リポジトリは、その手順で作成した配布版である。
...
- [x] ソースコードと文書は MIT License とし、ルートに `LICENSE` を置く
- [x] 配布サンプルの設定とプロンプトも MIT License の対象にする
- [x] README の「ライセンス」を確定内容へ更新する

利用者が後から追加する画像、文章、その他の素材は、各素材の権利と利用サービスの規約に従う。
